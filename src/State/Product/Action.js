...
export const findProducts = (reqData) => async (dispatch) => {
    ...
    try {
     ...
     // const formattedColors = colors.length > 0 ? colors.map(encodeURIComponent).join(',') : '';
      const formattedSizes = Array.isArray(sizes) ? sizes.map(encodeURIComponent).join(',') : '';
...
export const findProductById = (reqData) => async (dispatch) => {
    try {
        ...
        console.log("reqt data"+reqData)


        const {
            data
        } = await api.get(`/api/products/id/${reqData.productdId}`);
